...
export const BulkMoveModal: React.FC<BulkMoveModalProps> = ({
  ...
  const handleBulkMove = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Failed to move selected files');

      onShowToast(`Moved ${fileIds.length} files successfully on VPS storage`);
      onSuccess();
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-md w-full p-6 shadow-2xl shadow-black/80 relative text-zinc-100">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-white">Bulk Move Files</h3>
            <p className="text-xs text-zinc-400">
              Moving {fileIds.length} selected files on VPS storage
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="space-y-1.5 max-h-56 overflow-y-auto pr-1 custom-scrollbar">
              ...
              <div
                ...
                className={`p-2.5 rounded-lg border flex items-center gap-2.5 cursor-pointer text-xs transition-all active:scale-[0.99] ${
                  targetFolderId === 'root'
                    ? 'bg-orange-950/40 border-orange-500/80 text-orange-400 font-bold'
